  ...
  if (device_api == reshade::api::device_api::vulkan) {
    ...
    for (const auto& [shader_hash, custom_shader] : custom_shaders) {
      ...
      for (const auto& view_binding : custom_shader.views) {
        ...
        // Ignore DX12 defaults
        if (view_binding.space == 50u) continue;
...
static void MergeVulkanDescriptorParameters(
    ...
    const std::vector<uint32_t>& existing_set_param_indices,
    reshade::api::pipeline_layout_param* new_params) {
  const auto existing_sets = static_cast<uint32_t>(existing_set_param_indices.size());
  for (uint32_t set_index = 0; set_index < injected_params.size() && set_index < existing_sets; ++set_index) {
    ...
    auto& original_param = new_params[existing_set_param_indices[set_index]];
...
static std::vector<uint32_t> GetVulkanDescriptorSetParamIndices(
    const reshade::api::pipeline_layout_param* params,
    uint32_t param_count) {
  std::vector<uint32_t> indices;
  indices.reserve(param_count);
...
static bool EnsureVulkanDescriptorTable(
    reshade::api::device* device,
    renodx::utils::shader::PipelineShaderDetails* pipeline_details,
    uint32_t layout_param,
    bool* allocated_table = nullptr) {
  if (allocated_table != nullptr) *allocated_table = false;
  if (pipeline_details == nullptr) return false;
  if (pipeline_details->injection_layout.handle == 0u) return false;

  if (layout_param < pipeline_details->descriptor_tables.size()
      && pipeline_details->descriptor_tables[layout_param].handle != 0u) {
    return true;
  }

  reshade::api::descriptor_table table = {0u};
  utils::pipeline_layout::GetPipelineLayoutData(pipeline_details->layout, [&](const auto* pipeline_data) {
    if (layout_param < pipeline_data->descriptor_tables.size()) {
      table = pipeline_data->descriptor_tables[layout_param];
    }
  });

  if (table.handle == 0u) {
    const bool allocated = device->allocate_descriptor_table(pipeline_details->injection_layout, layout_param, &table);
    if (!allocated || table.handle == 0u) {
      std::stringstream s;
      s << "mods::shader::EnsureVulkanDescriptorTable(";
      s << "failed to allocate descriptor table";
      s << ", layout: " << PRINT_PTR(pipeline_details->injection_layout.handle);
      s << ", set: " << layout_param;
    ...
    utils::pipeline_layout::UpdatePipelineLayoutData(pipeline_details->layout, [&](utils::pipeline_layout::PipelineLayoutData& pipeline_data) {
      if (pipeline_data.descriptor_tables.size() <= layout_param) {
        pipeline_data.descriptor_tables.resize(layout_param + 1u);
      }
      pipeline_data.descriptor_tables[layout_param] = table;
    });
    if (allocated_table != nullptr) *allocated_table = true;
  }

  if (pipeline_details->descriptor_tables.size() <= layout_param) {
    pipeline_details->descriptor_tables.resize(layout_param + 1u);
  }
  pipeline_details->descriptor_tables[layout_param] = table;
  return true;
}
...
static bool OnCreatePipelineLayout(
  ...
  if (has_descriptor_injection && is_vulkan) {
    const auto existing_set_param_indices = GetVulkanDescriptorSetParamIndices(params, old_count);

    MergeVulkanDescriptorParameters(data->injected_descriptor_params, existing_set_param_indices, new_params);
...
static void OnInitPipelineLayout(
  ...
  if (device_api == reshade::api::device_api::d3d9) {
    ...
    if (data->use_pipeline_layout_cloning) {
      ...
      const auto existing_set_param_indices = is_vulkan
                                                  ? GetVulkanDescriptorSetParamIndices(params, old_count)
                                                  : std::vector<uint32_t>{};
      const auto existing_set_count = static_cast<uint32_t>(existing_set_param_indices.size());
      uint32_t added_descriptor_param_count = 0u;
      if (has_descriptor_injection) {
        if (is_vulkan) {
          added_descriptor_param_count = data->injected_descriptor_params.size() > existing_set_count
...

  {
    ...
    utils::pipeline_layout::UpdatePipelineLayoutData(layout, [&](utils::pipeline_layout::PipelineLayoutData& pipeline_data) {
      ...
      pipeline_data.descriptor_tables.clear();
...
static void OnDestroyPipelineLayout(
  ...
  utils::pipeline_layout::GetPipelineLayoutData(layout, [&](const auto* pipeline_data) {
    bool has_descriptor_tables = false;
    for (const auto table : pipeline_data->descriptor_tables) {
    ...
    if (has_descriptor_tables) {
      changed = true;
      FreeDescriptorTables(device, pipeline_data->descriptor_tables);
    }
  });
      ...
      if (use_pipeline_layout_cloning && !applied_replacement) {
        ...
        while (span_begin < pending_view_push_count) {
          ...
          if (use_descriptor_tables) {
            if (!EnsureVulkanDescriptorTable(context.cmd_list->get_device(), state.pipeline_details, first_view_push.layout_param)) {
#ifdef DEBUG_LEVEL_0
              std::stringstream s;
              s << "mods::shader::OnCommandAction(";
...
inline void OnPresent(
  ...
  if (push_injections_on_present) {
    ...
    if (state->last_pipeline != 0u) {
      ...
      if (details != nullptr) {
        if (details->injection_layout != 0u && details->injection_index != -1) {
          const uint32_t injection_offset = constant_buffer_offset != 0
                                                ? constant_buffer_offset
