...
## Material ID Encoding (from GBuffer RT2.w)

```glsl
uint packedByte = uint(roundEven(gbuffer2.w * 255.0));  // _614 in decompiled code
uint materialType = packedByte & 7u;                     // _615 — bits 0-2, BRDF switch
uint lightLayer   = (packedByte & 112u) >> 4u;           // _617 — bits 4-6, per-light visibility mask
```

| ID | Material | Key indicator in shader |
|----|----------|------------------------|
| 0 | Standard PBR | Falls through (no special branch) |
| 1 | Unlit/Sky | Immediate `break` |
| 2 | Eye/Iris | Secondary octahedral normal from RT3, `_686` |
| 3 | Subsurface | `_674`, mixes albedo with white |
| 4 | Hair | Marschner Gaussian BRDF, `_694` |
| 5 | Clearcoat | Dual normal decode from RT3, `_633` |
| 6 | Anisotropic | Tangent from RT3, split roughness alpha, `_690` |
| 7 | Cloth | Ashikhmin/Charlie sheen, hero buffer lookup via `_12` |

**Note:** The classification shader routes tiles based on which material IDs are present.
`_617` (light layer mask) is used ONLY for per-light visibility: `lightData._m6 & (1u << (_617 & 31u))`.
It is NOT the material type selector.
...
8. **Group A fast-path:** Add `#ifdef LAYOUT_GROUP_A` early-out before the full material
   switch — `if (_615 != 0u) { break; }`. Group A only ever receives standard PBR tiles,
   so this prevents ~5000 lines of dead material branches from polluting the instruction
   cache. Does not affect correctness (the dispatch only sends ID-0 tiles to Group A),
   but prevents potential GPU occupancy regression vs the original 1930-line shader.
9. **CSM debug guard:** Wrap `pc.csm_debug` visualization code in `#ifdef LAYOUT_GROUP_C`.
   Without this, the debug path would attempt to sample `_8` (texture2DArray) which
   doesn't exist in Group A/B, producing undefined results.
10. **Body file must NOT contain `#version 450`** — that line lives only in the wrapper
    files. Body starts directly with `#extension` directives.
...
|-------|---------------|
| Material ID = `_614 & 7u` (low 3 bits) | VERIFIED (code in 0x3B67DBD8, matches 07-MATERIALS doc) |
| Light layer = `(_614 & 112u) >> 4u` (bits 4-6) | VERIFIED (used for `_m6 & (1u << _617)` only) |
...
2. **Validation layers:** Run with `VK_LAYER_KHRONOS_validation` enabled on first
   deploy to catch any descriptor set binding mismatch (especially `set=0/b=0` in
   Group C shaders 0x92395FFB and 0xA4582498). If validation errors appear, split
   Group C into C1 (3 shaders with b=0) and C2 (2 without) — adds 2 more wrappers.
3. Deploy, load game, check for tile seams at material boundaries
4. Key test areas:
...
5. Toggle `pc.hero_lighting` via RenoDX UI to verify Group C hero loop
6. CSM debug view (`pc.csm_debug`) to confirm cascade sampling in Group C
   and verify it does NOT produce garbage in Group A/B tiles
7. Performance spot-check: compare GPU frame time with uber-shader vs old per-shader
   in a heavy scene (Act 3 lower city). Group A regression is the main risk.
