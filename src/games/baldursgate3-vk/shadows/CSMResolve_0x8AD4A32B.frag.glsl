...
// IS-FAST noise (128×128×32 R8G8_UNORM texture array) — injected by addon via push_descriptors
// Importance-Sampled Filter-Adapted Spatio-Temporal noise for TAA-convergent PCSS sampling
...
void main()
{
    ...
    float _130 = _129.x;
    vec3 _131 = _4 * _130;
    // Shadow sampling noise source — toggled via RenoDX UI
    float _158;
    if (pc.rendering_isfast_noise > 0.5)
    {
        // IS-FAST: 128×128×32 EMA-optimized spatiotemporal noise (EA SEED)
        // Indexed by (pixelXY % 128, sampleIndex % 32) — the texture is pre-optimized
        // for exponential moving average convergence under TAA.
        uint _isFastFrame = uint(_6._m2);
        vec2 _isFastSample = texelFetch(isFastNoise, ivec3(ivec2(gl_FragCoord.xy) & 127, int(_isFastFrame & 31u)), 0).rg;
        _158 = _isFastSample.r;
    }
    else
    {
        // Vanilla: engine's Bayer dither + temporal offset
        _158 = fma(fract(_6._m2 * 0.25), _6._m3, fma(0.2700000107288360595703125, fma(((_125 & 1) == 0) ? 1.0 : (-1.0), 0.5, ((_124 & 1) == 0) ? 1.0 : (-1.0)) + 1.5, texelFetch(_18, ivec3(ivec2(gl_FragCoord.xy * 0.5) & ivec2(63), 0).xy, 0).x));
    }
    ...
    // Mode 0: Vanilla derivative normals + vanilla bias
    // Mode 1: FFXVI Oriented Depth Bias (GBuffer normals, linear-space bias, no material-type hacks)
    // Mode 2: Disable CSM (outputs 1.0) — isolates non-CSM shadow systems
    // Mode 3: Zero bias diagnostic — tests if bias amplifies flicker
    // Mode 4: Max taps — tests if sparse sampling causes flicker
    vec3 _186_deriv = normalize(_171).xyz;
    int _rdx_shadow_mode = int(pc.rendering_shadow_improvements + 0.5);

    if (_rdx_shadow_mode == 2)
    {
        _5 = vec4(1.0);
        return;
    }

    // --- FFXVI Oriented Depth Bias: decode GBuffer normal for Mode 1 ---
    // Used for orientation-aware bias direction. Stable across frames (no TAA jitter dependency).
    vec3 _rdx_gbuf_normal = _186_deriv;  // fallback for non-Mode-1
    if (_rdx_shadow_mode == 1)
    {
        ...
        _rdx_gbuf_normal = normalize(viewNormal * mat3(_7._m6[0].xyz, _7._m6[1].xyz, _7._m6[2].xyz));
    }

    vec3 _186;
    if (_rdx_shadow_mode == 3)
    {
        _186 = vec3(0.0, 0.0, 1.0);
    }
    else if (_rdx_shadow_mode == 1)
    {
        // Mode 1: Use GBuffer normals for the vanilla normal-offset path too (for PCSS disk offset).
        // Blend toward derivative at edges to avoid cross-geometry bias bleed.
        _186 = normalize(mix(_rdx_gbuf_normal, _186_deriv, _183));
    }
    else
    {
        _186 = _186_deriv;
    ...
    if (_191 || _188)
    {
        ...
        if (_188)
        {
            ...
            bool _233;
            SPIRV_CROSS_BRANCH
            // Material type 4 inversion now handled at source (GBuffer shader 0x0A136550
            // changed to write type 0 for these opaque meshes). CSMResolve logic unchanged.
            if (_212 || (_211 == 4))
            {
    ...
    do
    {
        ...
        if (_369)
        {
            ...
            if (_rdx_shadow_mode == 1)
            {
                // Vanilla bias magnitudes + GBuffer normals + no material-type-4 inversion.
                // Uses stable GBuffer normals instead of jittery derivative normals,
                // and never inverts bias for material type 4 (the proven fix for acne).
                vec3 _403 = _186 * mat3(_13._m10[_367][0].xyz, _13._m10[_367][1].xyz, _13._m10[_367][2].xyz);
                // _186 is already GBuffer-normal-derived in Mode 1 (set above).
                // _235 is already false for material type 4 (Hypothesis A exclusion above).
                vec3 _407 = _235 ? -_403 : _403;
            ...
            {
                // Mode 0/3/4: Vanilla bias computation
                vec3 _403 = _186 * mat3(_13._m10[_367][0].xyz, _13._m10[_367][1].xyz, _13._m10[_367][2].xyz);
                vec3 _407;
                if (_235)
                {
        ...
        do
        {
            ...
            if (_369)
            {
                // Mode 4: force 14 blocker taps (vs vanilla 7-_367)
                int _448 = (_rdx_shadow_mode == 4) ? 14 : (7 - _367);
                float _450;
                float _453;
                _450 = 0.0;
                _453 = 0.0;
        ...
        if (_369)
        {
            ...
            // Mode 4: force 18 PCF taps (vs vanilla 9-_367)
            int _532 = (_rdx_shadow_mode == 4) ? 18 : (9 - _367);
            float _534;
            _534 = 0.0;
        ...
        if (_rdx_shadow_mode == 1)
        {
            // Mode 1: Vanilla magnitudes + GBuffer normals, no type-4 inversion
            vec3 _633 = _235 ? -_629 : _629;
        ...
        {
            // Vanilla bias for second cascade
            vec3 _633;
            if (_235)
            {
                _633 = -_629;
        ...
        // Mode 4: force 18 PCF taps for second cascade too (vs vanilla 8-_367)
        int _658 = (_rdx_shadow_mode == 4) ? 18 : (8 - _367);
        float _660;
        _660 = 0.0;
