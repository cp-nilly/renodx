#ifndef RENODX_SHADERS_TONEMAP_PSYCHOV_TEST20_HLSL_
#define RENODX_SHADERS_TONEMAP_PSYCHOV_TEST20_HLSL_

#include "./shared.h"

// Included via shared.h -> renodx.hlsl at compile time
...
float3 psychotm_test20(
...
#if 0  // bleaching disabled — renodx::color::bleaching not yet in library
  if (false && bleaching_intensity != 0.f) {
    float stimulus_lm = max(lms_cones.x + lms_cones.y, 0.f);
    float adaptive_yf = max(renodx::color::yf::from::LMS(current_adaptive_state_lms), 1e-6f);
    float clip_scale = renodx::math::DivideSafe(clip_point, adaptive_yf, 0.f);
    float clip_lm = max(
        (current_adaptive_state_lms.x + current_adaptive_state_lms.y) * clip_scale,
        0.f);

    float bleach_availability =
        psycho20_BleachAvailabilityFromClipPoint(
            stimulus_lm,
            clip_lm);
    ...
    float3 availability =
        lerp(
            1.f.xxx,
            bleach_availability.xxx,
            saturate(bleaching_intensity));

    lms_cones = max(
        0.f.xxx,
        renodx::color::bleaching::ApplyAvailabilityToLMSPerConeWhiteRelative(
            lms_cones,
            availability,
            current_adaptive_state_lms));
  }
...

  {
    ...
    float m_bias_weight = 0.f;  // disabled for testing — was: spectral_confidence * (0.12f * l_bias + 0.025f * s_bias);
...
#endif  // RENODX_SHADERS_TONEMAP_PSYCHOV_TEST20_HLSL_
